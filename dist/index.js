...
async function main() {
    ...
    const orchestrator = new Orchestrator(new FileStateStore(join(dataDir, "state.json")), new Supervisor(), new CommandRunner(), new WorkerRunner(), join(dataDir, "run.lock"));
...
async function runNext(orchestrator) {
...
async function runUntilBlocked(orchestrator, args) {
    ...
    console.log("Remaining ready tasks:", new Supervisor().runIteration(finalState.plan).readyTaskIds.join(", ") || "(none)");
...
async function completeTask(orchestrator, args) {
...
async function listProfiles(profileStore) {
    const profiles = await profileStore.list();
    for (const profile of profiles) {
        console.log(`${profile.id}: ${profile.name}`);
    }
}
async function useProfile(orchestrator, profileStore, args) {
...
async function runValidation(orchestrator) {
...
async function showPlan(orchestrator) {
    const state = await orchestrator.loadState();
...
async function loadPlan(orchestrator, args) {
    ...
    const state = mode === "preserve-runtime"
...
async function exportPlan(orchestrator, args) {
    const [planPath] = args;
    if (!planPath) {
...
async function resetPlan(orchestrator, defaultPlanStore) {
...
async function addTask(orchestrator, args) {
    ...
    if (!taskId || !type || !priorityValue || !estimatedScopeValue || !title) {
        console.error('Usage: npm run add-task -- <task-id> <type> <priority> <estimated-scope> "<title>" [description] [dep1,dep2] [criterion1|criterion2]');
        process.exitCode = 1;
        return;
    }
...
async function setTaskStatus(orchestrator, args) {
    ...
    const state = await orchestrator.updateTaskStatus(taskId, status);
...
async function setTaskDependencies(orchestrator, args) {
...
async function bootstrapRepo(orchestrator, profileStore, args) {
...
async function analyzeActiveRepo(orchestrator) {
    const state = await orchestrator.loadState();
...
async function seedPlanFromAnalysis(orchestrator) {
    const state = await orchestrator.loadState();
    const analysis = await analyzeRepo(state.workspace.repoPath, state.profile);
    ...
    console.log("Ready tasks:", new Supervisor().runIteration(nextState.plan).readyTaskIds.join(", ") || "(none)");
}
async function writeArchitectureSummary(orchestrator) {
    const state = await orchestrator.loadState();
    const analysis = await analyzeRepo(state.workspace.repoPath, state.profile);
...
async function reviewActiveProfile(orchestrator) {
    const state = await orchestrator.loadState();
    const analysis = await analyzeRepo(state.workspace.repoPath, state.profile);
...
function collectTaskIds(plan, status) {
