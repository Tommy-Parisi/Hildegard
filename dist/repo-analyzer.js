...
export async function analyzeRepo(repoPath, profile) {
    ...
    const topLevelFiles = visibleEntries
        ...
        .map((entry) => entry.name)
        .sort();
    const manifests = manifestCandidates.filter((name) => topLevelFiles.includes(name));
    ...
    return {
        ...
        manifests,
        subsystems,
        summary: createSummary(repoPath, profile, topLevelDirectories, topLevelFiles, manifests, subsystems),
    };
}
export function createArchitectureSummary(analysis) {
...
export function reviewProfileFit(analysis, profile) {
    const hasValidationCommands = profile.commands.some((command) => command.category === "validation");
    const profileMatchesPackageJson = profile.id === "typescript-node" && analysis.manifests.includes("package.json");
    const profileMatchesCargo = profile.id === "rust-generic" && analysis.manifests.includes("Cargo.toml");
...
export function createSeedTasks(analysis) {
...
function createSummary(repoPath, profile, topLevelDirectories, topLevelFiles, manifests, subsystems) {
...
async function discoverSubsystems(repoPath, profile) {
    const subsystemHints = [];
