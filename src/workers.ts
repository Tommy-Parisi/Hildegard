...
export class WorkerRunner {
  ...
  async assign(task: TaskNode): Promise<WorkerAssignment | null> {
    const workerKind = chooseWorkerKind(task);
  ...
  async run(assignment: WorkerAssignment, state: OrchestrationState): Promise<WorkerResult> {
...
function chooseWorkerKind(task: TaskNode): WorkerKind | null {
  switch (task.type) {
    case "research":
      return "research";
    case "planning":
...
function supportsPlannerTask(taskId: string): boolean {
  return (
...
function supportsValidationTask(taskId: string): boolean {
  return taskId === "task_run_validation" || taskId === "task_validation_baseline_followup";
}

function createFirstWorkSliceSummary(workspaceName: string, subsystems: string[]): string {
...
function createFirstWorkSliceTasks(subsystems: string[]): TaskNode[] {
  const focusArea = subsystems[0] ?? "src";
...
function toTaskSlug(value: string): string {
...
async function runBaselineValidation(
  ...
  const results = await Promise.all(
    validationCommands.map((command) =>
      commandRunner.run(command, state.plan.approvalPolicy),
    ),
  );
...
function summarizeValidationBaseline(state: OrchestrationState): WorkerResult {
  const validationHistory = state.executionHistory.filter(
    (record) =>
      record.taskId === "task_run_validation" || record.taskId.startsWith("validation:"),
  );
...
function summarizeValidationResults(results: CommandExecutionResult[]): string {
  return results
